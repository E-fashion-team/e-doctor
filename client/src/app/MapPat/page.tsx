...
const MapComponent: React.FC = () => {
  ...
  const getAllLoc = () => {
    axios
      .get("http://localhost:5000/api/docloc/getAll")
  ...
  const showDocLocation = () => {
    if (mapRef.current) {
      array.forEach(
        (e: { latitude: number; longitude: number; name: string }) => {
  ...
  return (
    <div>
      ...
      <div className="btn-add">
        <button className="button-add" onClick={() => getAllLoc()}>
          Get all doctors location{" "}
        </button>

        <button className="button-add" onClick={() => getLocalisation()}>
          get my location
        </button>
        <button className="button-add" onClick={() => checkAllDist()}>
