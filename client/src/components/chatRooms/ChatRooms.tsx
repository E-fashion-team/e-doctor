...
const ChatRooms = ({ setUpdate, update }: props) => {
    ...
    try {
      ...
      if (type === "doctor") {
        const res = await axios.get(
          `http://localhost:5000/api/room/getAllDoc/${doctor.id}`
        );
        setChatRooms(res.data);
      } else if (type === "patient") {
        const res = await axios.get(
          `http://localhost:5000/api/room/getAllPat/${patient.id}`
        );
        setChatRooms(res.data);
      }
    } catch (error) {
                    ...
                    <div className="d-flex flex-row">
                      <div>
                        <img
                          src={
                            type === "doctor"
                              ? room.Patient.avatarUrl
                              : room.Doctor.avatarUrl
                          }
                          alt="avatar"
