...
const Conversation = ({ udpate }: any) => {
  ...
  const roomId: any =
    typeof localStorage !== "undefined" ? localStorage.getItem("roomId") : null;
  ...
  const handleGetMessages = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/room/OneRoom/${roomId}`
      );
  ...
  const handleSendMessage = async () => {
    if (message.trim()) {
      ...
      setMessage("");
      try {
        const res = await axios.post("http://localhost:5000/api/message/add/", {
          content: message,
          senderPhone,
          DoctorId,
          PatientId,
    ...
    <div className="col-md-6 col-lg-7 col-xl-8">
      ...
      >
        ...
        {msg?.map((message: any, i: number) => {
          ...
          const reciver =
            message.senderPhone != doctor.phone ? doctor : patient;
            ...
            >
              ...
              <div>
                <p
                  ...
                  style={{ backgroundColor: left ? "#f5f6f7" : "blue" }}
                >
                  {message.content}
                </p>
